...
const VersusPanel = ({ left, right, trickRoom }: VersusProps) => {
  ...
  if (!comparison) {
    return (
      <Card className="text-center text-sm text-muted-foreground">양쪽 포켓몬을 입력하면 선공을 판정한다.</Card>
    );
  }
  ...
  const column = (accent: Accent, side: SpeedSide, value: number, winner: boolean) => {
    ...
    return (
      ...
      >
        ...
        {winner ? <Badge variant="success" size="sm">선공</Badge> : <span className="h-[22px]" />}
      </div>
    );
  };
    ...
    <Card className="flex flex-col gap-4">
      <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3">
        ...
        <div className="flex flex-col items-center gap-1">
          <span className="text-xs font-bold tracking-[0.3em] text-muted-foreground">VS</span>
          {comparison.faster === "tie" ? (
            <Badge variant="muted">동률</Badge>
          ) : (
            <span className="text-xs text-muted-foreground">+{gap}</span>
          )}
          {trickRoom && <Badge variant="warning" size="sm">트릭룸</Badge>}
        </div>
      ...
      <div className="flex h-2 overflow-hidden rounded-full bg-muted" aria-hidden>
        ...
        <div className="flex-1 bg-info transition-all" />
      </div>

      <p className="text-center text-sm">
        {comparison.faster === "tie" ? (
          <span className="font-semibold text-muted-foreground">동률 — 50% 선공</span>
        ) : (
          <>
...
const SideCard = ({ title, accent, side, winning, onChange }: SideCardProps) => {
  ...
  return (
    <Card className={cn("flex flex-col gap-3 transition", winning && "ring-1 ring-primary/40")}>
      <div className="flex items-center gap-2">
        ...
        {winning && <Badge variant="success" size="sm">선공</Badge>}
