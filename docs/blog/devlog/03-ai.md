...
이 과정은 두 번의 방향 전환을 거쳤다.

## 첫 번째 시행착오: 클립보드 왕복 UX

초기 설계는 클립보드 paste 왕복이었다. 앱이 파티 정보를 직렬화해 보여주면, 사용자가 그걸
복사해 Claude에 붙여넣고, 응답을 다시 앱에 붙여넣는 방식이었다. export 버튼과 paste
사이드패널을 양방향 슬롯으로 만들었다.

돌이켜보면 과한 우회였다. 사용자가 매번 복사·붙여넣기를 두 번 해야 했고, 응답 형식이 조금만
어긋나도 파싱이 깨졌다. 결국 이 플로우는 폐기했다.

## 전환: 서버가 Anthropic을 직접 호출

대신 서버가 Anthropic API를 직접 호출하도록 바꿨다. 프롬프트 본문은 도메인 계층의
`serializeForClaude`로 만들고, 응답은 Structured Outputs로 스키마를 강제했다. 느슨한 파싱
대신 Zod 스키마(`ClaudeResponseSchema`)에 맞는 구조화 응답을 받는다.
...
정적 시스템 프롬프트에는 prompt caching을 걸어, 같은 사용자의 연속 호출에서 입력 토큰 비용을
줄였다.
...
`mentionedNames`로 함께 반환하게 하고, 이를 검증 사전(`isKnownTerm`)과 대조한다. 사전에 없는
이름이 있으면 그 목록을 다시 모델에 주고 한 번 더 교정을 요청한다.

```ts
let unverified = parsed.mentionedNames.filter((name) => !isKnownTerm(name));
if (unverified.length > 0) {
  const correction = `\n\n## 명칭 교정 요청\n다음 명칭은 검증 사전에 없다: ${unverified.join(', ')}\n정식 한국 명칭으로 바꾸거나, 확신이 없으면 해당 언급을 삭제하고 다시 작성하라.`;
...
AI를 붙이는 일은 "API를 호출한다"가 전부가 아니었다. 어떤 UX로 데이터를 주고받을지(왕복 폐기),
어떤 모델이 한국어 도메인에 맞는지(Sonnet), 그리고 모델이 틀린 이름을 낼 때 어떻게 잡을지
(2패스 보정)가 실제 일의 대부분이었다. 다음 편은 이 모든 걸 떠받칠 백엔드를 NestJS로 다시
세운 이야기다.
