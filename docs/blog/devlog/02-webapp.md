...
프로젝트 전체 톤을 한 곳에서 통제하기 위해서였다. 종족 입력칸 옆에 뜨는 미니 스프라이트도
같은 맥락이다 — 입력이 도감의 실제 종족으로 인식됐는지를 아이콘 등장 여부로 즉시 피드백한다.
오타면 아이콘이 안 뜬다.

## 5. AI 없이 보여줄 수 있는 데까지는 AI 없이

파티빌더 상단에는 파티를 입력하는 즉시 갱신되는 분석 패널이 붙어 있다. 2배 이상으로 받는
타입별 약점 멤버 수, 약점 분산 점수, 역할 분포, 화력 합계(물리·특수·내구) 같은 것들이다.
이건 AI가 아니라 1편의 결정론 공식이 브라우저에서 직접 계산한다. 입력할 때마다 공짜로,
즉시, 항상 같은 값이 나온다.

이 분리는 비용 문제이기도 하지만 신뢰 문제이기도 했다. 상성표를 뒤지면 누구나 검산할 수
있는 값을 굳이 모델에게 시키면, 모델이 가끔 틀릴 때 전체 분석의 신뢰가 같이 무너진다.
그래서 **계산으로 끝나는 것은 전부 결정론 계층이 맡고, AI에게는 계산이 안 되는 판단만
넘긴다**가 이 앱의 일관된 분업이 됐다. 나중에 AI를 붙일 때(3편) 이 패널의 계산 결과가
프롬프트에 "이미 계산된 값, 재계산 불필요"로 그대로 실리면서, 이 분업은 화면 구성을 넘어
프롬프트 설계 원칙이 된다.
