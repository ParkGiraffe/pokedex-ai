...
> **상태:** 확정 (2026-05-21, Phase 2 완료 후 재검토).
> **Phase 0~2 반영 결정:**
> - **점수 모델은 기술셋 비의존**: 상대 기술셋·EV·도구는 미상이므로 정확한 데미지 KO 판정 대신 **타입 상성(자속) + 스피드(상대는 최대 투자 가정) + 종족값**으로 결정론적 매치업 verdict(유리/불리/호각)를 낸다. spec §4의 `myKO2/expectedDamageRange`는 이 비의존 모델로 대체(유연성 §10 "점수 모델이 거칠면 가중치만 바꾼다").
> - **UI는 옵션 B(별도 `/matchup` 페이지)**: 내 파티는 파티빌더 store 재사용, 상대는 종족명만 입력(공개분). 매치업 매트릭스 표시.
> - **부분 정보**: 상대 공개분(1~6마리)만으로 동작. 비공개 슬롯 메타 가중은 메타 데이터 수집 후로 미룸(현재 메타 빈 상태).
> - 메타가 비어 있어도 동작. `serializeForClaude("matchup-leadrec")`에 매치업 매트릭스를 첨부한다.
