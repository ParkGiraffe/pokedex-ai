...
> **상태:** 윤곽 (outline). Phase 1 구현이 끝난 뒤 이 spec을 다시 검토·확정한 다음 `writing-plans` 사이클로 진입한다.
> **유연성 원칙:** 현재 가정 기준 윤곽이다. Phase 0~1 구현 중 발견되는 사실(파티 빌더 UX 실측, 메타 데이터 가용성, Claude 응답 패턴)에 따라 자유롭게 수정한다.
