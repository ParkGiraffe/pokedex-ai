---
description: React Native 개발 규칙을 설명한다.
paths:
  - 'apps/mobile/**'
---
...
1. 기본적인 코드 작성 원칙은 React Native 공식 문서를 따른다.
2. React Native 메커니즘을 우회하지 않는다.
...
- 타입 선언은 `interface` 대신 `type` 을 권장한다.
  ...
  - React 내장 타입은 `type` 으로 정의되어 있어 `&` 조합이 자연스럽다.
    - 예시 2: `type CardProps = { children: ReactNode } & ViewProps`
...
- Context의 속성은 값 및 변경 함수, 이벤트, 스타일, DOM 참조 순으로 정렬한다.
  - 예시 1:
    ...
    type SelectContextValue = {
      ...
      onChange?: (value: string) => void;
      size: SelectSize;
      selectTriggerRef: RefObject<View | null>;
    };
    ```
- 컴포넌트는 바깥에서 안쪽으로의 데이터 흐름에 따라 도구, 외부 입력, 서버 상태, 로컬 상태, 파생 값, 핸들러, 이펙트 순으로 작성한다.
  - 예시 1:
    ```tsx
    const { t } = useTranslation();
    const navigate = useNavigate();
    const phoneNumber = useLocation({ select: (location) => location.state.phoneNumber ?? '' });
    const completeOtpSignIn = useCompleteOtpSignIn();
