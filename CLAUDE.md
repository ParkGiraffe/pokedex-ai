...
- **모델 선택은 작업 특성에 맞춤.** 이미지 OCR(`/import-party`)은 Opus 4.7 (정확도). 파티 분석(`/analyze-party`)은 Haiku 4.5 (응답 속도). 매치업·배틀(`/matchup-leadrec`, `/battle-advice`)은 Sonnet 4.6 (분석 깊이). 임의 격하 금지지만 작업별 권장 모델은 위와 같다.
